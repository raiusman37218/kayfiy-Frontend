    ...
    <article className="group">
      <Link href={`/products/${slug(product.name)}`} className="block">
        <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-blush shadow-sm transition-all duration-500 group-hover:-translate-y-1.5 group-hover:shadow-[0_20px_40px_-20px_rgba(43,39,36,0.4)]">
          ...
          {onSale && (
            <span className="absolute top-3 left-3 rounded-full bg-[#7A2A3D] px-3 py-1 text-[10px] font-bold tracking-[0.14em] text-white uppercase shadow-sm animate-pulse-soft">
              Sale
            </span>
          )}
          {onSale && (
          ...
          {isAvailable && (
            <button
              ...
              className="absolute bottom-0 inset-x-0 flex items-center justify-center gap-1.5 bg-charcoal/90 backdrop-blur-sm py-3 text-[11px] font-bold tracking-wider text-white uppercase translate-y-full transition-all duration-300 group-hover:translate-y-0 hover:bg-[#7A2A3D] cursor-pointer"
            >
        ...
        <h3 className="mt-3 line-clamp-2 text-sm font-medium text-charcoal transition group-hover:text-[#7A2A3D]">
          {product.name}
        </h3>
